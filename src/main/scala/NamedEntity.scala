...
class University(text: String) extends Organization(text) {
  override def entityType: String = "University"        // como la clase padre de university SI tiene definido entityType, tenemos que especificar que la sobreescriba (override)
}
